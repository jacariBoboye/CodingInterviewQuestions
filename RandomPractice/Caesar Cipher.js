...
letter in a string by a fixed number, , making it unreadable by his enemies. Given a string, , and a number, , encrypt  
...
function cearsersCipher(s, k){

	var stringBuilder = ""

	for (var i = 0; i< s.length;i++){
		if((s.charCodeAt(i) >= 65 && s.charCodeAt(i) <= 90) || (s.charCodeAt(i) >= 97 && s.charCodeAt(i) <= 122)){
			if((s.charCodeAt(i) + k) >= 122) {stringBuilder = stringBuilder + String.fromCharCode(s.charCodeAt(i) + k - 26); continue;}
			if((s.charCodeAt(i) + k) >= 90){stringBuilder = stringBuilder + String.fromCharCode(s.charCodeAt(i) + k - 26); continue;}
			stringBuilder = stringBuilder + String.fromCharCode(s.charCodeAt(i) + k)
			continue
		}
		stringBuilder = stringBuilder + String.fromCharCode(s.charCodeAt(i) + k)
		}
...
console.log(cearsersCipher(str, 2), "Expected: okffng-Qwvb")
